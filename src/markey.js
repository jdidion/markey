...
function parseMarkeyFile(options) {
    ...
    if (frontYAML = docParts[2]) {
        docOpts = yamlParser.load(frontYAML);
    }
    content = docParts[3] || '';
    ...
    return { docOpts: docOpts, sections: sections };
}

function mergeTemplate(mkData, options) {
    // determine stylesheet and html template to use
    style = mkData.docOpts.style || options.defaultStyle;
    template = mkData.docOpts.template || options.defaultTemplate;
